...
public class FileFunctionExecutor implements FunctionExecutor{
	...
	public static void download(String path, List<String> urls) throws IOException{
		if(!CollectionUtils.isEmpty(urls)) {
			for (String url : urls) {
				FileUtils.downloadFile(path, url,null, true);
			}
		}
	}

	@Comment("下载Url资源")
	@Example("${file.download('e:/downloadPath',urls,name)}")
	public static void download(String path, String url,String name) throws IOException {
		if (url != null) {
			FileUtils.downloadFile(path, url, name,true);
		}
	}
